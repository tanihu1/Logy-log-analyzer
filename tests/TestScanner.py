...
class TestBasicUsage(unittest.TestCase):
    ...
    def test_count_event(self):
        ...
        result = scanner.scan_log_file(LOG_FILE)
        self.assertEqual(len(result),1,"Scanner detected incorrect amount of log lines!");
        self.assertEqual(result[0],"Event: TELEMETRY count - matches: 2 entries", "Scanner returned wrong result for count event")
