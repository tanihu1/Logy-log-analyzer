...
class TestBasicExecution(unittest.TestCase):
    def test_valid_line(self):
        ...
        self.assertEqual(
            result.type, "TELEMETRY", "Config parser detected incorrect type of event!"
        )
        self.assertTrue(result.count, "Config parser detected wrong 'count' flag!")
        self.assertEqual(
            result.pattern,
            "^Iteration time:\\s\\d+\\.\\d+\\ssec$",
            "Config parser detected incorrect regex!",
        )
    ...
    def test_detectes_invalid_flag(self):
        ...
        try:
            ...
            self.assertTrue(
                e.error_type == ConfigError.FLAG_ERROR,
                "Config parser raised wrong exception!",
            )
